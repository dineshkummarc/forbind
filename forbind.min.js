...
"]").replace(/(?:^|:|,)(?:\s*\[)+/g,""))){j=eval("("+text+")");return typeof reviver==="function"?walk({"":j},""):j}throw new SyntaxError("JSON.parse");}})();this.io={version:"0.6",setPath:function(path){if(window.console&&console.error)console.error("io.setPath will be removed. Please set the variable WEB_SOCKET_SWF_LOCATION pointing to WebSocketMain.swf");this.path=/\/$/.test(path)?path:path+"/";WEB_SOCKET_SWF_LOCATION=path+"lib/vendor/web-socket-js/WebSocketMain.swf"}};
if("jQuery"in this)jQuery.io=this.io;if(typeof window!="undefined")WEB_SOCKET_SWF_LOCATION=(document.location.protocol=="https:"?"https:":"http:")+"//cdn.socket.io/"+this.io.version+"/WebSocketMain.swf";
(function(){var _pageLoaded=false;io.util={ios:false,load:function(fn){if(document.readyState=="complete"||_pageLoaded)return fn();if("attachEvent"in window)window.attachEvent("onload",fn);else window.addEventListener("load",fn,false)},inherit:function(ctor,superCtor){for(var i in superCtor.prototype)ctor.prototype[i]=superCtor.prototype[i]},indexOf:function(arr,item,from){for(var l=arr.length,i=from<0?Math.max(0,l+from):from||0;i<l;i++)if(arr[i]===item)return i;return-1},isArray:function(obj){return Object.prototype.toString.call(obj)===
"[object Array]"}};io.util.ios=/iphone|ipad/i.test(navigator.userAgent);io.util.android=/android/i.test(navigator.userAgent);io.util.opera=/opera/i.test(navigator.userAgent);io.util.load(function(){_pageLoaded=true})})();
(function(){var frame="~m~",stringify=function(message){if(Object.prototype.toString.call(message)=="[object Object]"){if(!("JSON"in window)){if("console"in window&&console.error)console.error("Trying to encode as JSON, but JSON.stringify is missing.");return'{ "$error": "Invalid message" }'}return"~j~"+JSON.stringify(message)}else return String(message)};Transport=io.Transport=function(base,options){this.base=base;this.options={timeout:15E3};for(var i in options)if(this.options.hasOwnProperty(i))this.options[i]=
options[i]};Transport.prototype.send=function(){throw new Error("Missing send() implementation");};Transport.prototype.connect=function(){throw new Error("Missing connect() implementation");};Transport.prototype.disconnect=function(){throw new Error("Missing disconnect() implementation");};Transport.prototype._encode=function(messages){var ret="",message,messages=io.util.isArray(messages)?messages:[messages];for(var i=0,l=messages.length;i<l;i++){message=messages[i]===null||messages[i]===undefined?
"":stringify(messages[i]);ret+=frame+message.length+frame+message}return ret};Transport.prototype._decode=function(data){var messages=[],number,n;do{if(data.substr(0,3)!==frame)return messages;data=data.substr(3);number="",n="";for(var i=0,l=data.length;i<l;i++){n=Number(data.substr(i,1));if(data.substr(i,1)==n)number+=n;else{data=data.substr(number.length+frame.length);number=Number(number);break}}messages.push(data.substr(0,number));data=data.substr(number)}while(data!=="");return messages};Transport.prototype._onData=
function(data){this._setTimeout();var msgs=this._decode(data);if(msgs&&msgs.length)for(var i=0,l=msgs.length;i<l;i++)this._onMessage(msgs[i])};Transport.prototype._setTimeout=function(){var self=this;if(this._timeout)clearTimeout(this._timeout);this._timeout=setTimeout(function(){self._onTimeout()},this.options.timeout)};Transport.prototype._onTimeout=function(){this._onDisconnect()};Transport.prototype._onMessage=function(message){if(!this.sessionid){this.sessionid=message;this._onConnect()}else if(message.substr(0,
3)=="~h~")this._onHeartbeat(message.substr(3));else if(message.substr(0,3)=="~j~")this.base._onMessage(JSON.parse(message.substr(3)));else this.base._onMessage(message)},Transport.prototype._onHeartbeat=function(heartbeat){this.send("~h~"+heartbeat)};Transport.prototype._onConnect=function(){this.connected=true;this.connecting=false;this.base._onConnect();this._setTimeout()};Transport.prototype._onDisconnect=function(){this.connecting=false;this.connected=false;this.sessionid=null;this.base._onDisconnect()};
...
(function(){var empty=new Function,XMLHttpRequestCORS=function(){if(!("XMLHttpRequest"in window))return false;var a=new XMLHttpRequest;return a.withCredentials!=undefined}(),request=function(xdomain){if("XDomainRequest"in window&&xdomain)return new XDomainRequest;if("XMLHttpRequest"in window&&(!xdomain||XMLHttpRequestCORS))return new XMLHttpRequest;if(!xdomain){try{var a=new ActiveXObject("MSXML2.XMLHTTP");return a}catch(e){}try{var b=new ActiveXObject("Microsoft.XMLHTTP");return b}catch(e){}}return false},
XHR=io.Transport.XHR=function(){io.Transport.apply(this,arguments);this._sendBuffer=[]};io.util.inherit(XHR,io.Transport);XHR.prototype.connect=function(){this._get();return this};XHR.prototype._checkSend=function(){if(!this._posting&&this._sendBuffer.length){var encoded=this._encode(this._sendBuffer);this._sendBuffer=[];this._send(encoded)}};XHR.prototype.send=function(data){if(io.util.isArray(data))this._sendBuffer.push.apply(this._sendBuffer,data);else this._sendBuffer.push(data);this._checkSend();
return this};XHR.prototype._send=function(data){var self=this;this._posting=true;this._sendXhr=this._request("send","POST");this._sendXhr.onreadystatechange=function(){var status;if(self._sendXhr.readyState==4){self._sendXhr.onreadystatechange=empty;try{status=self._sendXhr.status}catch(e){}self._posting=false;if(status==200)self._checkSend();else self._onDisconnect()}};this._sendXhr.send("data="+encodeURIComponent(data))},XHR.prototype.disconnect=function(){this._onDisconnect();return this};XHR.prototype._onDisconnect=
function(){if(this._xhr){this._xhr.onreadystatechange=this._xhr.onload=empty;this._xhr.abort();this._xhr=null}if(this._sendXhr){this._sendXhr.onreadystatechange=this._sendXhr.onload=empty;this._sendXhr.abort();this._sendXhr=null}this._sendBuffer=[];io.Transport.prototype._onDisconnect.call(this)};XHR.prototype._request=function(url,method,multipart){var req=request(this.base._isXDomain());if(multipart)req.multipart=true;req.open(method||"GET",this._prepareUrl()+(url?"/"+url:""));if(method=="POST"&&
"setRequestHeader"in req)req.setRequestHeader("Content-type","application/x-www-form-urlencoded; charset=utf-8");return req};XHR.check=function(xdomain){try{if(request(xdomain))return true}catch(e){}return false};XHR.xdomainCheck=function(){return XHR.check(true)};XHR.request=request})();
(function(){var WS=io.Transport.websocket=function(){io.Transport.apply(this,arguments)};io.util.inherit(WS,io.Transport);WS.prototype.type="websocket";WS.prototype.connect=function(){var self=this;this.socket=new WebSocket(this._prepareUrl());this.socket.onmessage=function(ev){self._onData(ev.data)};this.socket.onclose=function(ev){self._onClose()};return this};WS.prototype.send=function(data){this.socket.send(this._encode(data));return this};WS.prototype.disconnect=function(){this.socket.close();
return this};WS.prototype._onClose=function(){this._onDisconnect();return this};WS.prototype._prepareUrl=function(){return(this.base.options.secure?"wss":"ws")+"://"+this.base.host+":"+this.base.options.port+"/"+this.base.options.resource+"/"+this.type+(this.sessionid?"/"+this.sessionid:"")};WS.check=function(){return"WebSocket"in window&&WebSocket.prototype&&WebSocket.prototype.send&&!!WebSocket.prototype.send.toString().match(/native/i)&&typeof WebSocket!=="undefined"};WS.xdomainCheck=function(){return true}})();
...
args)});return this};Flashsocket.prototype._onClose=function(){if(!this.base.connected){this.base.options.transports.splice(io.util.indexOf(this.base.options.transports,"flashsocket"),1);this.base.transport=this.base.getTransport();this.base.connect();return}return io.Transport.websocket.prototype._onClose.call(this)};Flashsocket.check=function(){if(typeof WebSocket=="undefined"||!("__addTask"in WebSocket))return false;if(io.util.opera)return false;if("navigator"in window&&"plugins"in navigator&&
navigator.plugins["Shockwave Flash"])return!!navigator.plugins["Shockwave Flash"].description;if("ActiveXObject"in window)try{return!!(new ActiveXObject("ShockwaveFlash.ShockwaveFlash")).GetVariable("$version")}catch(e){}return false};Flashsocket.xdomainCheck=function(){return true}})();
(function(){var HTMLFile=io.Transport.htmlfile=function(){io.Transport.XHR.apply(this,arguments)};io.util.inherit(HTMLFile,io.Transport.XHR);HTMLFile.prototype.type="htmlfile";HTMLFile.prototype._get=function(){var self=this;this._open();window.attachEvent("onunload",function(){self._destroy()})};HTMLFile.prototype._open=function(){this._doc=new ActiveXObject("htmlfile");this._doc.open();this._doc.write("<html></html>");this._doc.parentWindow.s=this;this._doc.close();var _iframeC=this._doc.createElement("div");
this._doc.body.appendChild(_iframeC);this._iframe=this._doc.createElement("iframe");_iframeC.appendChild(this._iframe);this._iframe.src=this._prepareUrl()+"/"+ +new Date};HTMLFile.prototype._=function(data,doc){this._onData(data);var script=doc.getElementsByTagName("script")[0];script.parentNode.removeChild(script)};HTMLFile.prototype._destroy=function(){this._iframe.src="about:blank";this._doc=null;CollectGarbage()};HTMLFile.prototype.disconnect=function(){this._destroy();return io.Transport.XHR.prototype.disconnect.call(this)};
HTMLFile.check=function(){if("ActiveXObject"in window)try{var a=new ActiveXObject("htmlfile");return a&&io.Transport.XHR.check()}catch(e){}return false};HTMLFile.xdomainCheck=function(){return false}})();
(function(){var XHRMultipart=io.Transport["xhr-multipart"]=function(){io.Transport.XHR.apply(this,arguments)};io.util.inherit(XHRMultipart,io.Transport.XHR);XHRMultipart.prototype.type="xhr-multipart";XHRMultipart.prototype._get=function(){var self=this;this._xhr=this._request("","GET",true);this._xhr.onreadystatechange=function(){if(self._xhr.readyState==3)self._onData(self._xhr.responseText)};this._xhr.send()};XHRMultipart.check=function(){return"XMLHttpRequest"in window&&"prototype"in XMLHttpRequest&&
"multipart"in XMLHttpRequest.prototype};XHRMultipart.xdomainCheck=function(){return true}})();
(function(){var empty=new Function,XHRPolling=io.Transport["xhr-polling"]=function(){io.Transport.XHR.apply(this,arguments)};io.util.inherit(XHRPolling,io.Transport.XHR);XHRPolling.prototype.type="xhr-polling";XHRPolling.prototype.connect=function(){if(io.util.ios||io.util.android){var self=this;io.util.load(function(){setTimeout(function(){io.Transport.XHR.prototype.connect.call(self)},10)})}else io.Transport.XHR.prototype.connect.call(this)};XHRPolling.prototype._get=function(){var self=this;this._xhr=
this._request(+new Date,"GET");if("onload"in this._xhr)this._xhr.onload=function(){self._onData(this.responseText);self._get()};else this._xhr.onreadystatechange=function(){var status;if(self._xhr.readyState==4){self._xhr.onreadystatechange=empty;try{status=self._xhr.status}catch(e){}if(status==200){self._onData(self._xhr.responseText);self._get()}else self._onDisconnect()}};this._xhr.send()};XHRPolling.check=function(){return io.Transport.XHR.check()};XHRPolling.xdomainCheck=function(){return io.Transport.XHR.xdomainCheck()}})();
...
JSONPPolling.prototype._send=function(data){var self=this;if(!("_form"in this)){var form=document.createElement("FORM"),area=document.createElement("TEXTAREA"),id=this._iframeId="socket_io_iframe_"+this._index,iframe;form.style.position="absolute";form.style.top="-1000px";form.style.left="-1000px";form.target=id;form.method="POST";form.action=this._prepareUrl()+"/"+ +new Date+"/"+this._index;area.name="data";form.appendChild(area);this._insertAt.parentNode.insertBefore(form,this._insertAt);document.body.appendChild(form);
this._form=form;this._area=area}function complete(){initIframe();self._posting=false;self._checkSend()}function initIframe(){if(self._iframe)self._form.removeChild(self._iframe);try{iframe=document.createElement('<iframe name="'+self._iframeId+'">')}catch(e){iframe=document.createElement("iframe");iframe.name=self._iframeId}iframe.id=self._iframeId;self._form.appendChild(iframe);self._iframe=iframe}initIframe();this._posting=true;this._area.value=data;try{this._form.submit()}catch(e){}if(this._iframe.attachEvent)iframe.onreadystatechange=
function(){if(self._iframe.readyState=="complete")complete()};else this._iframe.onload=complete};JSONPPolling.prototype._get=function(){var self=this,script=document.createElement("SCRIPT");if(this._script){this._script.parentNode.removeChild(this._script);this._script=null}script.async=true;script.src=this._prepareUrl()+"/"+ +new Date+"/"+this._index;script.onerror=function(){self._onDisconnect()};this._insertAt.parentNode.insertBefore(script,this._insertAt);this._script=script};
JSONPPolling.prototype._=function(){this._onData.apply(this,arguments);this._get();return this};JSONPPolling.check=function(){return true};JSONPPolling.xdomainCheck=function(){return true};
(function(){var Socket=io.Socket=function(host,options){this.host=host||document.domain;this.options={secure:false,document:document,port:document.location.port||80,resource:"socket.io",transports:["websocket","flashsocket","htmlfile","xhr-multipart","xhr-polling","jsonp-polling"],transportOptions:{"xhr-polling":{timeout:25E3},"jsonp-polling":{timeout:25E3}},connectTimeout:5E3,tryTransportsOnConnectTimeout:true,rememberTransport:true};for(var i in options)if(this.options.hasOwnProperty(i))this.options[i]=
options[i];this.connected=false;this.connecting=false;this._events={};this.transport=this.getTransport();if(!this.transport&&"console"in window)console.error("No transport available")};Socket.prototype.getTransport=function(override){var transports=override||this.options.transports,match;if(this.options.rememberTransport&&!override){match=this.options.document.cookie.match("(?:^|;)\\s*socketio=([^;]*)");if(match)transports=[decodeURIComponent(match[1])]}for(var i=0,transport;transport=transports[i];i++)if(io.Transport[transport]&&
io.Transport[transport].check()&&(!this._isXDomain()||io.Transport[transport].xdomainCheck()))return new io.Transport[transport](this,this.options.transportOptions[transport]||{});return null};Socket.prototype.connect=function(){if(this.transport&&!this.connected){if(this.connecting)this.disconnect();this.connecting=true;this.transport.connect();if(this.options.connectTimeout){var self=this;setTimeout(function(){if(!self.connected){self.disconnect();if(self.options.tryTransportsOnConnectTimeout){var remainingTransports=
...
return this};Socket.prototype.on=function(name,fn){if(!(name in this._events))this._events[name]=[];this._events[name].push(fn);return this};Socket.prototype.fire=function(name,args){if(name in this._events)for(var i=0,ii=this._events[name].length;i<ii;i++)this._events[name][i].apply(this,args===undefined?[]:args);return this};Socket.prototype.removeEvent=function(name,fn){if(name in this._events)for(var a=0,l=this._events[name].length;a<l;a++)if(this._events[name][a]==fn)this._events[name].splice(a,
...
"socketio="+encodeURIComponent(this.transport.type);this.fire("connect")};Socket.prototype._onMessage=function(data){this.fire("message",[data])};Socket.prototype._onDisconnect=function(){var wasConnected=this.connected;this.connected=false;this.connecting=false;this._queueStack=[];if(wasConnected)this.fire("disconnect")};Socket.prototype.addListener=Socket.prototype.addEvent=Socket.prototype.addEventListener=Socket.prototype.on})();
...
af=/webkit/.test(ah)?parseFloat(ah.replace(/^.*webkit\/(\d+(\.\d+)?).*$/,"$1")):false,X=!+"1",ag=[0,0,0],ab=null;if(typeof t.plugins!=D&&typeof t.plugins[S]==r){ab=t.plugins[S].description;if(ab&&!(typeof t.mimeTypes!=D&&t.mimeTypes[q]&&!t.mimeTypes[q].enabledPlugin)){T=true;X=false;ab=ab.replace(/^.*\s+(\S+\s+\S+$)/,"$1");ag[0]=parseInt(ab.replace(/^(.*)\..*$/,"$1"),10);ag[1]=parseInt(ab.replace(/^.*\.(.*)\s.*$/,"$1"),10);ag[2]=/[a-zA-Z]/.test(ab)?parseInt(ab.replace(/^.*[a-zA-Z]+(.*)$/,"$1"),10):
0}}else if(typeof O.ActiveXObject!=D)try{var ad=new ActiveXObject(W);if(ad){ab=ad.GetVariable("$version");if(ab){X=true;ab=ab.split(" ")[1].split(",");ag=[parseInt(ab[0],10),parseInt(ab[1],10),parseInt(ab[2],10)]}}}catch(Z){}return{w3:aa,pv:ag,wk:af,ie:X,win:ae,mac:ac}}(),k=function(){if(!M.w3)return;if(typeof j.readyState!=D&&j.readyState=="complete"||typeof j.readyState==D&&(j.getElementsByTagName("body")[0]||j.body))f();if(!J){if(typeof j.addEventListener!=D)j.addEventListener("DOMContentLoaded",
...
function FABridge(target,bridgeName){this.target=target;this.remoteTypeCache={};this.remoteInstanceCache={};this.remoteFunctionCache={};this.localFunctionCache={};this.bridgeID=FABridge.nextBridgeID++;this.name=bridgeName;this.nextLocalFuncID=0;FABridge.instances[this.name]=this;FABridge.idMap[this.bridgeID]=this;return this}FABridge.TYPE_ASINSTANCE=1;FABridge.TYPE_ASFUNCTION=2;FABridge.TYPE_JSFUNCTION=3;FABridge.TYPE_ANONYMOUS=4;FABridge.initCallbacks={};FABridge.userTypes={};
FABridge.addToUserTypes=function(){for(var i=0;i<arguments.length;i++)FABridge.userTypes[arguments[i]]={typeName:arguments[i],enriched:false}};FABridge.argsToArray=function(args){var result=[];for(var i=0;i<args.length;i++)result[i]=args[i];return result};function instanceFactory(objID){this.fb_instance_id=objID;return this}
...
t=="string"||t=="boolean"||t==null||t==undefined)result=value;else if(value instanceof Array){result=[];for(var i=0;i<value.length;i++)result[i]=this.serialize(value[i])}else if(t=="function"){result.type=FABridge.TYPE_JSFUNCTION;result.value=this.getFunctionID(value)}else if(value instanceof ASProxy){result.type=FABridge.TYPE_ASINSTANCE;result.value=value.fb_instance_id}else{result.type=FABridge.TYPE_ANONYMOUS;result.value=value}return result},deserialize:function(packedValue){var result;var t=typeof packedValue;
if(t=="number"||t=="string"||t=="boolean"||packedValue==null||packedValue==undefined)result=this.handleError(packedValue);else if(packedValue instanceof Array){result=[];for(var i=0;i<packedValue.length;i++)result[i]=this.deserialize(packedValue[i])}else if(t=="object"){for(var i=0;i<packedValue.newTypes.length;i++)this.addTypeDataToCache(packedValue.newTypes[i]);for(var aRefID in packedValue.newRefs)this.createProxy(aRefID,packedValue.newRefs[aRefID]);if(packedValue.type==FABridge.TYPE_PRIMITIVE)result=
packedValue.value;else if(packedValue.type==FABridge.TYPE_ASFUNCTION)result=this.getFunctionProxy(packedValue.value);else if(packedValue.type==FABridge.TYPE_ASINSTANCE)result=this.getProxy(packedValue.value);else if(packedValue.type==FABridge.TYPE_ANONYMOUS)result=packedValue.value}return result},addRef:function(obj){this.target.incRef(obj.fb_instance_id)},release:function(obj){this.target.releaseRef(obj.fb_instance_id)},handleError:function(value){if(typeof value=="string"&&value.indexOf("__FLASHERROR")==
0){var myErrorMessage=value.split("||");if(FABridge.refCount>0)FABridge.refCount--;throw new Error(myErrorMessage[1]);return value}else return value}};ASProxy=function(bridge,typeName){this.bridge=bridge;this.typeName=typeName;return this};
...
{};if(!(type in this.__events)){this.__events[type]=[];if("function"==typeof this["on"+type]){this.__events[type].defaultHandler=this["on"+type];this["on"+type]=this.__createEventHandler(this,type)}}this.__events[type].push(listener)};WebSocket.prototype.removeEventListener=function(type,listener,useCapture){if(!("__events"in this))this.__events={};if(!(type in this.__events))return;for(var i=this.__events.length;i>-1;--i)if(listener===this.__events[type][i]){this.__events[type].splice(i,1);break}};
WebSocket.prototype.dispatchEvent=function(event){if(!("__events"in this))throw"UNSPECIFIED_EVENT_TYPE_ERR";if(!(event.type in this.__events))throw"UNSPECIFIED_EVENT_TYPE_ERR";for(var i=0,l=this.__events[event.type].length;i<l;++i){this.__events[event.type][i](event);if(event.cancelBubble)break}if(false!==event.returnValue&&"function"==typeof this.__events[event.type].defaultHandler)this.__events[event.type].defaultHandler(event)};WebSocket.prototype.__handleMessages=function(){var arr=this.__flash.readSocketData();
for(var i=0;i<arr.length;i++){var data=decodeURIComponent(arr[i]);try{if(this.onmessage){var e;if(window.MessageEvent){e=document.createEvent("MessageEvent");e.initMessageEvent("message",false,false,data,null,null,window,null)}else e={data:data};this.onmessage(e)}}catch(e){console.error(e.toString())}}};WebSocket.prototype.__createEventHandler=function(object,type){return function(data){var event=new WebSocketEvent;event.initEvent(type,true,true);event.target=event.currentTarget=object;for(var key in data)event[key]=
data[key];object.dispatchEvent(event,arguments)}};function WebSocketEvent(){}WebSocketEvent.prototype.cancelable=true;WebSocketEvent.prototype.cancelBubble=false;WebSocketEvent.prototype.preventDefault=function(){if(this.cancelable)this.returnValue=false};WebSocketEvent.prototype.stopPropagation=function(){this.cancelBubble=true};WebSocketEvent.prototype.initEvent=function(eventTypeArg,canBubbleArg,cancelableArg){this.type=eventTypeArg;this.cancelable=cancelableArg;this.timeStamp=new Date};WebSocket.CONNECTING=
0;WebSocket.OPEN=1;WebSocket.CLOSING=2;WebSocket.CLOSED=3;WebSocket.__tasks=[];WebSocket.__initialize=function(){if(WebSocket.__swfLocation)window.WEB_SOCKET_SWF_LOCATION=WebSocket.__swfLocation;if(!window.WEB_SOCKET_SWF_LOCATION){console.error("[WebSocket] set WEB_SOCKET_SWF_LOCATION to location of WebSocketMain.swf");return}var container=document.createElement("div");container.id="webSocketContainer";container.style.position="absolute";if(WebSocket.__isFlashLite()){container.style.left="0px";container.style.top=
"0px"}else{container.style.left="-100px";container.style.top="-100px"}var holder=document.createElement("div");holder.id="webSocketFlash";container.appendChild(holder);document.body.appendChild(container);swfobject.embedSWF(WEB_SOCKET_SWF_LOCATION,"webSocketFlash","1","1","9.0.0",null,{bridgeName:"webSocket"},{hasPriority:true,allowScriptAccess:"always"},null,function(e){if(!e.success)console.error("[WebSocket] swfobject.embedSWF failed")});FABridge.addInitializationCallback("webSocket",function(){try{WebSocket.__flash=
...
if(!mimeType||!mimeType.enabledPlugin||!mimeType.enabledPlugin.filename)return false;return mimeType.enabledPlugin.filename.match(/flashlite/i)?true:false};window.webSocketLog=function(message){console.log(decodeURIComponent(message))};window.webSocketError=function(message){console.error(decodeURIComponent(message))};if(!window.WEB_SOCKET_DISABLE_AUTO_INITIALIZATION)if(window.addEventListener)window.addEventListener("load",WebSocket.__initialize,false);else window.attachEvent("onload",WebSocket.__initialize)})();
(function(host,port,undefined){WEB_SOCKET_SWF_LOCATION="http://"+host+":"+port+"/flash/WebSocketMainInsecure.swf";function each(obj,fn,context){for(var key in obj)if(hasOwnProperty.call(obj,key))fn.call(context,obj[key],key,obj)}function trigger(){var args=[].slice.apply(arguments),event=args.shift();if(forbind.debug)console.log(event+", callbacks:"+(eventRegister[event]===undefined?0:eventRegister[event].length)+", event:"+JSON.stringify(args[0]));if(eventRegister[event]!==undefined)each(eventRegister[event],
function(fn){fn.apply(forbind,args)})}function ifconnected(fn){if(connected)fn.call(forbind);else forbind.connect(forbind.apikey,fn)}function messageHandler(msg){if(msg.data===undefined)msg.data={};msg.data.type=msg.type.substr(msg.type.indexOf(":")+1);if(msg.type&&msg.type.substr(0,4)==="app:")trigger(msg.type,msg.data);else if(msg.type&&msg.type.substr(0,5)==="user:")trigger(msg.data.type,msg.data);else console&&console.log("unknown message: (type:"+msg.type+") "+JSON.stringify(msg))}var user={},
config={},eventRegister={},connected=false,socket=new io.Socket(host,{port:port});var forbind={version:"0.1",debug:false,apikey:"",connect:function(apikey,callback){if(typeof apikey==="function"){callback=apikey;apikey=""}forbind.apikey=apikey+="";if(!connected){socket.on("connect",function(){connected=true;socket.send({method:"load",data:{apikey:apikey,user:user}})});socket.on("disconnect",function(){connected=false;trigger("disconnect",{})});socket.on("message",messageHandler);each(eventRegister,
function(fn,event){if(event.substr(0,4)==="app:"&&event!=="app:error")forbind.unbind(event)});this.bind("app:load",function(_config){config=_config;if(callback!==undefined)callback.call(forbind,config);trigger("connect",{config:config,type:"connect"})});socket.connect()}},connected:function(){return connected},bind:function(event,fn){if(typeof event==="string")if(/ /.test(event))each(event.split(" "),function(event){forbind.bind(event,fn)});else if(eventRegister[event]===undefined)eventRegister[event]=
[fn];else eventRegister[event].push(fn);else for(var key in event)this.bind(key,event[key]);return this},on:function(event,fn){return this.bind(event,fn)},unbind:function(event,fn){if(fn===undefined)delete eventRegister[event];else{var eventHandlers=[];each(eventRegister[event],function(handler){if(handler!=fn)eventHandlers.push(handler)});eventRegister[event]=eventHandlers}return this},join:function(key,readonlykey){ifconnected(function(){socket.send({method:"app:join",data:{user:user,key:key,readonlykey:readonlykey}})})},
create:function(key){key=key||(+new Date).toString(32);ifconnected(function(){this.bind("app:create",function(){this.join(key)});socket.send({method:"app:create",data:{key:key}})});return key},leave:function(){ifconnected(function(){socket.send({method:"app:leave"})})},users:function(callback){ifconnected(function(){forbind.unbind("app:users").on("app:users",callback);socket.send({method:"app:users"})})},pass:function(){ifconnected(function(){socket.send({method:"user:pass"})})},send:function(message){ifconnected(function(){socket.send({method:"user:message",
data:message,user:user})})},user:function(details){user=details}};forbind.bind("app:error",function(data){if(eventRegister.error!==undefined)trigger("error",data);else throw new Error(data.description);});var lastChild;if(typeof this.document!=="undefined"){var scripts=document.getElementsByTagName("script");lastChild=scripts[scripts.length-1];if(lastChild.nodeName==="SCRIPT")lastChild.getAttribute("src").replace(/apikey=(.+?)\b/,function(n,apikey){forbind.apikey=apikey})}if(typeof exports!=="undefined")exports.forbind=
forbind;else window.forbind=forbind})("forbind.net",80);
