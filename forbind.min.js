...
[fn];else eventRegister[event].push(fn);else for(var key in event)this.bind(key,event[key]);return this},on:function(event,fn){return this.bind(event,fn)},unbind:function(event,fn){if(fn===undefined)delete eventRegister[event];else{var eventHandlers=[];each(eventRegister[event],function(handler){if(handler!=fn)eventHandlers.push(handler)});eventRegister[event]=eventHandlers}return this},join:function(key){ifconnected(function(){socket.send({method:"app:join",data:{user:user,key:key}})})},create:function(key){key=
key||(+new Date).toString(32);ifconnected(function(){this.bind("app:create",function(){this.join(key)});socket.send({method:"app:create",data:{key:key}})});return key},leave:function(){ifconnected(function(){socket.send({method:"app:leave"})})},users:function(callback){ifconnected(function(){forbind.unbind("app:users").on("app:users",callback);socket.send({method:"app:users"})})},pass:function(){ifconnected(function(){socket.send({method:"user:pass"})})},send:function(message){ifconnected(function(){socket.send({method:"user:message",
