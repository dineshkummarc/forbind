...
key||(+new Date).toString(32);ifconnected(function(){this.bind("app:create",function(){this.join(key)});socket.send({method:"app:create",data:{key:key}})});return key},leave:function(){ifconnected(function(){socket.send({method:"app:leave"})})},send:function(message){ifconnected(function(){socket.send({method:"user:message",data:message,user:user})})},user:function(details){user=details}};forbind.bind("app:error",function(data){if(eventRegister.error!==undefined)trigger("error",data);else throw new Error(data.description);
});var lastChild;if(typeof this.document!=="undefined"&&document.body){var scripts=document.getElementsByTagName("script");lastChild=scripts[scripts.length-1];if(lastChild.nodeName==="SCRIPT")lastChild.getAttribute("src").replace(/apikey=(.+?)\b/,function(n,apikey){forbind.apikey=apikey})}if(typeof exports!=="undefined")exports.forbind=forbind;else window.forbind=forbind})("forbind.net",80);
